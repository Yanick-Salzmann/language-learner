...
@Component({
  ...
  template: `
    <div class="message chat-message-container" [class.user]="message.sender === 'USER'" [class.ai]="message.sender === 'ASSISTANT'">
      <div class="message-avatar" [class.ai]="message.sender === 'ASSISTANT'">
        ...
        <div class="message-text" 
             *ngIf="message.sender === 'USER'">{{message.content.value}}</div>
        <markdown class="message-text markdown-content" 
             *ngIf="message.sender === 'ASSISTANT'" emoji [data]="message.content.asObservable() | async"></markdown>
...
export class MessageComponent implements OnInit {
  ...
  speakMessage(): void {
    if (this.message.content) {
      ...
      this.audioService.generateAndPlaySpeech(this.message.content.value, language);
    }
  }
